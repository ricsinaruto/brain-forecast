...
def test_run_main_dispatches_eval(monkeypatch, tmp_path):
    called = {}
    ...
    monkeypatch.setattr(run, "EvalQuant", DummyEval)
    monkeypatch.setattr(
        run,
        "eval_class_map",
        {"EvalQuant": DummyEval, "CustomEval": DummyEval},
    )

    cfg_path = _write_cfg(tmp_path, {"eval_class": "CustomEval"})

    run.main(["--args", str(cfg_path), "--mode", "eval"])

    assert called.get("ran") is True
    assert called["cfg"]["eval_class"] == "CustomEval"
