"""
Regression tests for ephys-gpt.
...
from ephys_gpt.dataset.datasplitter import (
    _chunk_sort_key,
    split_datasets,
)
from ephys_gpt.training.lightning import LitModel
...
class TestEmptyDatasetRegression:
    ...
    def test_empty_root_raises_error(self, tmp_path):
        ...
        from ephys_gpt.dataset.datasplitter import _prepare_datasets
...
class TestFreeRunConfigValidationRegression:
    ...
    def test_zero_warmup_rejected(self):
        ...
        from ephys_gpt.training.lightning import LitModelFreerun
        import torch.nn as nn
...
class TestAugmentationEdgeCases:
    """
    Regression: Augmentations should handle edge cases gracefully.
    """

    def test_time_warp_short_sequence(self):
        """TimeWarp should handle sequences shorter than num_anchors."""
        from ephys_gpt.dataset.augmentations import RandomTimeWarp

        torch.manual_seed(42)
        x = torch.randn(4, 3)  # Only 3 timesteps
        warp = RandomTimeWarp(num_anchors=6, p=1.0)  # More anchors than timesteps

        # Should not crash
        y = warp(x)
        assert y.shape == x.shape

    def test_time_mask_block_larger_than_sequence(self):
        """TimeMask should clamp block_len to sequence length."""
        from ephys_gpt.dataset.augmentations import RandomTimeMask

        torch.manual_seed(42)
        x = torch.randn(4, 10)  # 10 timesteps
        mask = RandomTimeMask(block_len=100, p=1.0)  # block >> sequence
