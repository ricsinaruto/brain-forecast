...
def spatial_temporal_resblk(x, block_s, block_t, temb):
    assert len(x.shape) == 5, "input should be 5D tensor, but got {}D tensor".format(
        len(x.shape)
    )
    B, C, T, H, W = x.shape
...
def make_attn(in_channels, use_checkpoint=False, norm_type="groupnorm"):
    return AttnBlockWrapper(
        in_channels, use_checkpoint=use_checkpoint, norm_type=norm_type
    )
...
class AttnBlockWrapper(AttnBlock):
    ...
    def attention(self, h_: torch.Tensor) -> torch.Tensor:
        ...
        b, c, t, h, w = q.shape
        q, k, v = map(
            lambda x: rearrange(x, "b c t h w -> b t (h w) c").contiguous(), (q, k, v)
        )
        h_ = torch.nn.functional.scaled_dot_product_attention(
            q, k, v
        )  # scale is dim ** -0.5 per default
...
class CausalConv1d(nn.Module):
    @beartype
    def __init__(
        self, chan_in, chan_out, kernel_size: int, pad_mode="constant", **kwargs
    ):
        super().__init__()
...
class Upsample(nn.Module):
    ...
    def forward(self, x):
        x = torch.nn.functional.interpolate(
            x.to(torch.float32), scale_factor=2.0, mode="nearest"
        ).to(x.dtype)
        if self.with_conv:
            x = self.conv(x)
        return x
...
class TimeUpsampleResCausal2x(nn.Module):
    ...
    def forward(self, x):
        ...
        if not self.enable_cached:
            ...
            self.causal_cache = x[
                :, :, -2 * self.num_temp_upsample: -self.num_temp_upsample
            ].clone()
            x = F.interpolate(
                x.to(torch.float32),
                scale_factor=[2.0, 1.0, 1.0],
...
class ResnetCausalBlock1D(nn.Module):
    def __init__(
        ...
        *,
        ...
        if self.in_channels != self.out_channels:
            if self.use_conv_shortcut:
                self.conv_shortcut = CausalConv1d(
                    in_channels, out_channels, kernel_size=3, stride=1
                )
            else:
                self.nin_shortcut = CausalConv1d(
...
class EncoderCausal3D(nn.Module):
    def __init__(
        ...
        **ignore_kwargs,
        ...
        self.spatial_ds = (
            list(range(0, self.num_resolutions - 1))
            if spatial_ds is None
            else spatial_ds
        )
        self.tempo_ds = (
            [self.num_resolutions - 2, self.num_resolutions - 3]
            if tempo_ds is None
            else tempo_ds
        )
        ...
        for i_level in range(self.num_resolutions):
            ...
            if i_level in self.spatial_ds:
                ...
                if i_level in self.tempo_ds:
                    down_temporal.downsample = TimeDownsampleResCausal2x(
                        block_in, block_in
                    )
...
class EncoderCausal3DPadding(EncoderCausal3D):
    ...
    def forward(self, x):
        ...
        if video_len % self.time_downsample_factor != 0:
            time_padding = (
                self.time_downsample_factor - video_len % self.time_downsample_factor
            )
            x = pad_at_dim(
                x, (time_padding, 0), dim=2, pad_mode=self.init_pad_mode, value=0.0
            )
...
class DecoderCausal3D(nn.Module):
    def __init__(
        ...
        **ignorekwargs,
        ...
        # upsampling
        self.spatial_us = (
            list(range(1, self.num_resolutions)) if spatial_us is None else spatial_us
        )
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            if i_level in self.tempo_us:
                up_temporal.upsample = TimeUpsampleResCausal2x(
                    block_in,
                    block_in,
                    interpolation_mode=self.interpolation_mode,
                    num_temp_upsample=num_temp_upsample,
                )
                num_temp_upsample *= 2
