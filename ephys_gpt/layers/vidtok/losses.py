...
from ..brainomni.loss import get_frequency_domain_loss, get_pcc
...
class GeneralLPIPSWithDiscriminator(nn.Module):
    def __init__(
        ...
        # BrainOmni temporal losses (applied across time dimension)
    ...
    def forward(
        ...
        if optimizer_idx == 0:
            ...
            # These losses operate on the time dimension (T) with H, W as batch dimensions
            ...
            if use_temporal_losses and self.dims > 2:
                # Rearrange (B, C, T, H, W) -> (B*H*W, C, 1, T)
                inp_temporal = rearrange(inputs, "b c t h w -> (b h w) c 1 t")
                rec_temporal = rearrange(reconstructions, "b c t h w -> (b h w) c 1 t")

                if self.temporal_pcc_weight > 0:
                    pcc = get_pcc(rec_temporal, inp_temporal)
            ...
            d_loss = d_weight * disc_factor * g_loss
            loss = nll_loss + d_loss
            ...
            log.update(
                {
                    ...
                    "{}/pcc".format(split): temporal_pcc_loss.detach(),
                    "{}/amp".format(split): temporal_freq_amp_loss.detach(),
                    "{}/phase".format(split): temporal_freq_phase_loss.detach(),
                }
            )
            return loss, log
