...
class ImageVideoLogger(Callback):
    def __init__(
        ...
        rescale=True,
        disabled=False,
        # log on batch_idx instead of global_step. global_step is fixed in validation. batch_idx restarts at each validation
        ...
        self.batch_freq_val = (
            batch_frequency_val if batch_frequency_val is not None else batch_frequency
        )
        ...
        for k in videos:
            ...
            if isinstance(videos[k], torch.Tensor) and videos[k].ndim == 5:
                ...
                if exists(pl_module):
                    ...
                    wandb.log(
                        {f"{split}/{k}": wandb.Video(frames, fps=self.video_fps)}
                    )  # k is str
    ...
    def log_img(self, pl_module, batch, batch_idx, split="train"):
        ...
        if (
            (
                ...
                or self.check_frequency_val(batch_idx, split)
            )
            ...
            with torch.no_grad(), torch.autocast(
                enabled=self.enable_autocast, device_type="cuda"
            ):
