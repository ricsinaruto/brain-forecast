...
def get_valid_dirs(
    dir1: str, dir2: str, dir3: Union[None, str] = None
) -> Union[None, str]:
...
def get_valid_paths(
    path1: str, path2: str, path3: Union[None, str] = None
) -> Union[None, str]:
...
def checkpoint(func, inputs, params, flag):
    ...
    :param params: a sequence of parameters `func` depends on but does not explicitly
        take as arguments.
...
def compute_ssim(x, y):
    ...
    ssim_map, cs_map = _compute_ssim_per_channel(
        x=x, y=y, kernel=kernel, data_range=1, k1=k1, k2=k2
    )
...
def _ssim_per_channel(
    ...
    Args:     x: An input tensor. Shape :math:`(N, C, H, W)`.     y: A target tensor.
    Shape :math:`(N, C, H, W)`.     kernel: 2D Gaussian kernel.     data_range: Maximum
    value range of images (usually 1.0 or 255).     k1: Algorithm parameter, K1 (small
    constant, see [1]).     k2: Algorithm parameter, K2 (small constant, see [1]).
    Try a larger K2 constant (e.g. 0.4) if you get a negative or NaN results.

    Returns:     Full Value of Structural Similarity (SSIM) index.
    """
...
def _ssim_per_channel_complex(
    ...
    Args:     x: An input tensor. Shape :math:`(N, C, H, W, 2)`.     y: A target tensor.
    Shape :math:`(N, C, H, W, 2)`.     kernel: 2-D gauss kernel.     data_range: Maximum
    value range of images (usually 1.0 or 255).     k1: Algorithm parameter, K1 (small
    constant, see [1]).     k2: Algorithm parameter, K2 (small constant, see [1]).
    Try a larger K2 constant (e.g. 0.4) if you get a negative or NaN results.

    Returns:     Full Value of Complex Structural Similarity (SSIM) index.
    """
    n_channels = x.size(1)
...
def gaussian_filter(
    ...
    r"""Returns 2D Gaussian kernel N(0,`sigma`^2) Args:

    kernel_size: Size of the kernel     sigma: Std of the distribution     device:
    target device for kernel generation     dtype: target data type for kernel
    generation Returns:     gaussian_kernel: Tensor with shape (1, kernel_size,
    kernel_size)
    """
