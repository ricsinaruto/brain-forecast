...
def view_range(
    x: torch.Tensor, i: int, j: int | None, shape: tuple[int, ...]
) -> torch.Tensor:
    """
...
def tensor_slice(x: torch.Tensor, begin: list[int], size: list[int]) -> torch.Tensor:
...
class AxialBlock(nn.Module):
    def __init__(self, n_hiddens, n_head):
        ...
        kwargs["causal"] = True
        kwargs["attn_type"] = "full"
        self.attn_t = MultiHeadAttention(attn_kwargs=dict(attn_dropout=0.0), **kwargs)
...
class Encoder(nn.Module):
    def __init__(self, n_hiddens, n_res_layers, downsample, in_channels: int = 3):
        super().__init__()
        ...
        for i in range(max_ds):
            in_ch = input_ch if i == 0 else n_hiddens
            stride = tuple([2 if d > 0 else 1 for d in n_times_downsample])
            conv = SamePadConv3d(in_ch, n_hiddens, 4, stride=stride)
            self.convs.append(conv)
            n_times_downsample -= 1
        # by now channels are n_hiddens
...
class Decoder(nn.Module):
    def __init__(self, n_hiddens, n_res_layers, upsample, out_channels: int = 3):
        super().__init__()
        self.res_stack = nn.Sequential(
            *[AttentionResidualBlock(n_hiddens) for _ in range(n_res_layers)],
            nn.BatchNorm3d(n_hiddens),
        ...
        for i in range(max_us):
            ch_out = out_channels if i == max_us - 1 else n_hiddens
            us = tuple([2 if d > 0 else 1 for d in n_times_upsample])
            convt = SamePadConvTranspose3d(n_hiddens, ch_out, 4, stride=us)
...
class SamePadConv3d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, bias=True):
        ...
        for i, p in enumerate(
            total_pad[::-1]
        ):  # reverse since F.pad starts from last dim
            if i == len(total_pad) - 1:
                # causal along time (depth) dimension: pad all on the left (past), none on the right (future)
                pad_input.append((p, 0))
            else:
                # spatial dims use symmetric padding
...
class AttentionStack(nn.Module):
    ...
    def forward(self, x, cond=None, decode_step=None, decode_idx=None):
        """
        Args
        ------
