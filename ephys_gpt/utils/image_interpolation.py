...
class GaussianSensorInterpolator:
    ...
    def __call__(self, values: torch.Tensor) -> torch.Tensor:
        ...
        weights = self.weights.to(values.device, values.dtype)
        weight_sum = self.weight_sum.to(values.device, values.dtype)

        img = torch.einsum("ct,chw->thw", values, weights)
        img = img / weight_sum
        return img
    ...
    def inverse(self, img: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            img: Tensor of shape (T, H, W) representing interpolated frames.

        Returns:
            values: Tensor of shape (C, T) with estimated sensor values.
        """
        img = img.squeeze()
        if img.ndim == 3:
            img = img.unsqueeze(0)
        ...
        weights = self.weights.to(img.device, img.dtype)  # (C, H, W)
        # Sum of weights for each sensor (for normalization)
        sensor_weight_sum = weights.sum(dim=(1, 2)).clamp_min(1e-6)  # (C,)

        # Weighted average of image pixels for each sensor
        values = torch.einsum("bthw,chw->bct", img, weights)
        values = values / sensor_weight_sum[None, :, None]
        return values
...
class GaussianSensorInverseInterpolator:
    ...
    GaussianSensorInterpolator.inverse() but can be instantiated independently
    when only the inverse operation is needed.
    """
